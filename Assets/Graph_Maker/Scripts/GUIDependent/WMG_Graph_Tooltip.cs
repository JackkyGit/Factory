...
public class WMG_Graph_Tooltip : WMG_GUI_Functions {
	...
	public void subscribeToEvents(bool val) {
		if (val) {
			...
			if(tooltipLabeler == null) //jzq - check the previous setting
	...
	void MouseEnterCommon(string textToSet, GameObject objToAnimate, Vector3 animTo) {
	...
	void MouseExitCommon(GameObject objToAnimate) {
	...
	void performTooltipAnimation (Transform trans, Vector3 newScale) {
