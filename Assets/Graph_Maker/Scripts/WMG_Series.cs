...
public class WMG_Series : MonoBehaviour {
	...
	[SerializeField] List<Vector2> _pointValues;
	public WMG_List<Vector2> pointValues = new WMG_List<Vector2>();
	[SerializeField] List<Color> _pointColors;
	...
	[SerializeField] comboTypes _comboType;
	[SerializeField] bool _useSecondYaxis;
	[SerializeField] string _seriesName;
	[SerializeField] float _pointWidthHeight;
	[SerializeField] float _lineScale;
	[SerializeField] Color _pointColor;
	[SerializeField] bool _usePointColors;
	[SerializeField] Color _lineColor;
	[SerializeField] bool _UseXDistBetweenToSpace;
	[SerializeField] bool _ManuallySetXDistBetween;
	[SerializeField] float _xDistBetweenPoints;
	[SerializeField] bool _ManuallySetExtraXSpace;
	[SerializeField] float _extraXSpace;
	[SerializeField] bool _hidePoints;
	[SerializeField] bool _hideLines;
	[SerializeField] bool _connectFirstToLast;
	[SerializeField] float _linePadding;
	[SerializeField] bool _dataLabelsEnabled;
	[SerializeField] int _dataLabelsNumDecimals;
	[SerializeField] int _dataLabelsFontSize;
	[SerializeField] Color _dataLabelsColor = Color.white;
	[SerializeField] FontStyle _dataLabelsFontStyle = FontStyle.Normal;
	[SerializeField] Font _dataLabelsFont; 
	[SerializeField] Vector2 _dataLabelsOffset;
	[SerializeField] areaShadingTypes _areaShadingType;
	[SerializeField] bool _areaShadingUsesComputeShader;
	[SerializeField] Color _areaShadingColor;
	[SerializeField] float _areaShadingAxisValue;
	[SerializeField] int _pointPrefab;
	[SerializeField] int _linkPrefab;
	...
	// Private vars
	UnityEngine.Object nodePrefab;
	List<GameObject> points = new List<GameObject>();
	List<GameObject> lines = new List<GameObject>();
	List<GameObject> areaShadingRects = new List<GameObject>();
	List<GameObject> dataLabels = new List<GameObject>();
	List<bool> barIsNegative = new List<bool>();
	List<int> changedValIndices = new List<int>();

	// Original property values for use with dynamic resizing
	public float origPointWidthHeight { get; set; }
	public float origLineScale { get; set; }
	public int origDataLabelsFontSize { get; set; }
	public Vector2 origDataLabelOffset { get; set; }
	
	// Cache
	WMG_Axis_Graph.graphTypes cachedSeriesType;
	
	// Real time update
	bool realTimeRunning;
	float realTimeLoopVar;
	float realTimeOrigMax;
	
	// Automatic Animation variables and functions
	bool animatingFromPreviousData;
	public bool currentlyAnimating { get; set; }
	List<Vector2> afterPositions = new List<Vector2>();
	List<int> afterWidths = new List<int>();
	List<int> afterHeights = new List<int>();
	//List<Vector2> previousPointValues = new List<Vector2>();

	List<WMG_Change_Obj> changeObjs = new List<WMG_Change_Obj>();
	public WMG_Change_Obj pointValuesC = new WMG_Change_Obj();
	public WMG_Change_Obj pointValuesCountC = new WMG_Change_Obj();
	WMG_Change_Obj pointValuesValC = new WMG_Change_Obj();
	WMG_Change_Obj lineScaleC = new WMG_Change_Obj();
	WMG_Change_Obj pointWidthHeightC = new WMG_Change_Obj();
	WMG_Change_Obj dataLabelsC = new WMG_Change_Obj();
	WMG_Change_Obj lineColorC = new WMG_Change_Obj();
	WMG_Change_Obj pointColorC = new WMG_Change_Obj();
	WMG_Change_Obj hideLineC = new WMG_Change_Obj ();
	WMG_Change_Obj hidePointC = new WMG_Change_Obj ();
	WMG_Change_Obj seriesNameC = new WMG_Change_Obj();
	WMG_Change_Obj linePaddingC = new WMG_Change_Obj();
	WMG_Change_Obj areaShadingTypeC = new WMG_Change_Obj ();
	WMG_Change_Obj areaShadingC = new WMG_Change_Obj ();
	public WMG_Change_Obj prefabC = new WMG_Change_Obj ();
	WMG_Change_Obj connectFirstToLastC = new WMG_Change_Obj ();
